...
void TraceThread::handleAccessMemory(Core::lock_signal_t lock_signal, Core::mem_op_t mem_op_type, IntPtr d_addr, char* data_buffer, UInt32 data_size)
{
   ...
   {
       LOG_PRINT("TraceThread::handleAccessMemory received CLFLUSH cmd!\n");
   }
   if (mem_op_type == Core::CLINVAL)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received CLINVAL cmd!\n");
   }
   if (mem_op_type == Core::NOT)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received NOT cmd!\n");
   }
   if (mem_op_type == Core::AND)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received AND cmd!\n");
   }
   if (mem_op_type == Core::OR)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received OR cmd!\n");
   }
   if (mem_op_type == Core::NAND)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received NAND cmd!\n");
   }
   if (mem_op_type == Core::NOR)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received NOR cmd!\n");
   }
   if (mem_op_type == Core::XOR)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received XOR cmd!\n");
   }
   if (mem_op_type == Core::CIMREAD)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received CIMREAD cmd!\n");
   }
   if (mem_op_type == Core::AND_S)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received AND_S cmd!\n");
   }
   if (mem_op_type == Core::OR_S)
   {
       LOG_PRINT("TraceThread::handleAccessMemory received OR_S cmd!\n");
   }
   ...
   switch (mem_op_type)
   {
      ...
      // add CIM cmd here
      case (Core::CLFLUSH):
         sift_mem_op = Sift::MemCLFLUSH;
         break;
      case (Core::CLINVAL):
         sift_mem_op = Sift::MemCLINVAL;
         break;
      case (Core::NOT):
         sift_mem_op = Sift::MemNOT;
         break;
      case (Core::AND):
         sift_mem_op = Sift::MemAND;
         break;
      case (Core::OR):
         sift_mem_op = Sift::MemOR;
         break;
      case (Core::NAND):
         sift_mem_op = Sift::MemNAND;
         break;
      case (Core::NOR):
         sift_mem_op = Sift::MemNOR;
         break;
      case (Core::XOR):
         sift_mem_op = Sift::MemXOR;
         break;
      case (Core::CIMREAD):
         sift_mem_op = Sift::MemCIMREAD;
         break;
      case (Core::AND_S):
         sift_mem_op = Sift::MemAND_S;
         break;
      case (Core::OR_S):
         sift_mem_op = Sift::MemOR_S;
         break;
