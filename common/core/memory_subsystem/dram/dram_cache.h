...
class DramCache : public DramCntlrInterface
{
   public:
      ...
      // added by MSON
      // just for compile error avoiding
      virtual boost::tuple<SubsecondTime, HitWhere::where_t> sendCimToMM(Core::mem_op_t mem_op_type, IntPtr address, core_id_t requester, Byte* data_buf, SubsecondTime now, ShmemPerf *perf);
