   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   if (mem_component == MemComponent::L1_ICACHE || mem_component == MemComponent::L1_DCACHE) {
      for(HitWhere::where_t hit_where = HitWhere::WHERE_FIRST; hit_where < HitWhere::NUM_HITWHERES; hit_where = HitWhere::where_t(int(hit_where)+1)) {
         ...
         // added by MSON
         registerStatsMetric(name, core_id, String("clflushes-where-")+where_str, &stats.clflushes_where[hit_where]);
      }
   }
...
// added by MSON
/*****************************************************************************
 * operations called by core on CLFLUSH cmd (ver3)
 *****************************************************************************/

HitWhere::where_t
CacheCntlr::processFlushReqFromCore(
      ...
      bool count)
{
    ...
    LOG_PRINT("processFlushReqFromCore(), lock_signal(%u), mem_op_type(%u), ca_address(0x%x)",
    ...
    //FIXMECacheBlockInfo *cache_block_info = getCacheBlockInfo(ca_address);
    CacheBlockInfo *cache_block_info;

    bool cache_hit = operationPermissibleinCache(ca_address, mem_op_type, &cache_block_info);

    hit_where = HitWhere::where_t(m_mem_component);

    if (cache_hit)
    {
        LOG_PRINT("CLFLUSH: L1 hit!\n");
       
        getMemoryManager()->incrElapsedTime(m_mem_component, CachePerfModel::ACCESS_CACHE_DATA_AND_TAGS, ShmemPerfModel::_USER_THREAD);

        if (cache_block_info->getCState() == CacheState::MODIFIED) // dirty cache
        {   
            //FIXME
            stats.clflushes_where[hit_where]++;
            
            LOG_PRINT("CLFLUSH: Writing back dirty CL in %s\n", HitWhereString(hit_where));
            SubsecondTime latency = SubsecondTime::Zero();
            
            std::pair<SubsecondTime, bool> res = updateCacheBlock(ca_address, CacheState::INVALID, Transition::COHERENCY, NULL, ShmemPerfModel::_USER_THREAD);
            ...
            LOG_ASSERT_ERROR(getCacheState(ca_address) == CacheState::INVALID, "Evicted address did not become invalid, now in state %s", CStateString(getCacheState(ca_address)));
            LOG_PRINT("%s flush done!\n", HitWhereString(hit_where));
        }
        else 
        {
            LOG_PRINT("CLFLUSH: No dirty CL in %s!\n", HitWhereString(hit_where));
        }
    }
    else
    {
        LOG_PRINT("CLFLUSH: L1 miss!\n");
    ...
    LOG_PRINT("Now go to the next level!\n");
    
    // check next level cache
    SubsecondTime t_issue = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
    hit_where = m_next_cache_cntlr->processFlushReqFromPrevCache(this, mem_op_type, ca_address, modeled, count, Prefetch::NONE, t_issue, false);
    ...
    // From here on downwards: not long anymore, only stats update so blanket cntrl lock
    {
        ...
        //FIXME
        //FIXMEstats.clflushes_where[hit_where]++;
    }
    ...
    LOG_PRINT("returning %s, latency %lu ns\n", HitWhereString(hit_where), total_latency.getNS());
    
    return hit_where;
}

HitWhere::where_t
CacheCntlr::processFlushReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
    ...
    Byte flush_buf[getCacheBlockSize()];
    
    //FIXMECacheBlockInfo *cache_block_info = getCacheBlockInfo(address);

    hit_where = HitWhere::where_t(m_mem_component);
    
    if (cache_hit)
    {
        LOG_PRINT("CLFLUSH: %s hit!\n", HitWhereString(hit_where));
        ...
        if ((cache_block_info->getCState() == CacheState::MODIFIED) && m_next_cache_cntlr) // dirty cache not in last-level cache
        {   
            //FIXME
            stats.clflushes_where[hit_where]++;
            ...
            std::pair<SubsecondTime, bool> res = updateCacheBlock(address, CacheState::INVALID, Transition::COHERENCY, NULL, ShmemPerfModel::_USER_THREAD);
            latency = getMax<SubsecondTime>(latency, res.first);
            
            getMemoryManager()->incrElapsedTime(latency, ShmemPerfModel::_USER_THREAD);
            atomic_add_subsecondtime(stats.snoop_latency, latency);
        
            LOG_ASSERT_ERROR(getCacheState(address) == CacheState::INVALID, "Evicted address did not become invalid, now in state %s", CStateString(getCacheState(address)));
            LOG_PRINT("%s flush done!\n", HitWhereString(hit_where));
        }
    ...
    else // !cache_hit: either data is not here, or operation on data is not permitted
    {
        ...
        if (cache_block_info && (cache_block_info->getCState() == CacheState::SHARED))
        {
            ...
            LOG_PRINT("CLFLUSH: Data is present, but still no cache_hit => this is a write on a SHARED block. Do Upgrade\n");
            SubsecondTime latency = SubsecondTime::Zero();
    ...
    if (m_next_cache_cntlr)
    {
        ...
        //FIXMEif (cache_block_info && (cache_block_info->getCState() == CacheState::MODIFIED))
        //FIXME    m_next_cache_cntlr->writeCacheBlock(address, 0, flush_buf, getCacheBlockSize(), ShmemPerfModel::_USER_THREAD);
        
        SubsecondTime t_issue = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
        hit_where = m_next_cache_cntlr->processFlushReqFromPrevCache(this, mem_op_type, address, modeled, count, Prefetch::NONE, t_issue, false);
    }
    else if (m_master->m_dram_cntlr)
    {
        if (cache_block_info && (cache_block_info->getCState() == CacheState::MODIFIED))
        {
            //FIXME
            stats.clflushes_where[hit_where]++;
            
            // get CL from last-level cache
            retrieveCacheBlock(address, flush_buf, ShmemPerfModel::_USER_THREAD, false);
            
            // Access DRAM
            SubsecondTime dram_latency;
            boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::WRITE, address, false, flush_buf);
            getMemoryManager()->incrElapsedTime(dram_latency, ShmemPerfModel::_USER_THREAD);
        }
    }

    LOG_PRINT("returning %s", HitWhereString(hit_where));
    return hit_where;
}

/*****************************************************************************
 * operations called by core on CIM cmd (ver1)
 *****************************************************************************/
HitWhere::where_t
CacheCntlr::processInvalReqFromCore(
      Core::lock_signal_t lock_signal,
      Core::mem_op_t mem_op_type,
      IntPtr ca_address, UInt32 offset,
      Byte* data_buf, UInt32 data_length,
      ...
      bool count)
{
    ...
    LOG_PRINT("processInvalReqFromCore(), lock_signal(%u), mem_op_type(%u), ca_address(0x%x)",
             lock_signal, mem_op_type, ca_address);
    ...
    LOG_PRINT("CLINVAL: invalidate in %s\n", HitWhereString(hit_where));
    // no cache_hit, invalidate CL for CIM cmds
    cache_hit = false;
    //FIXMEcache_block_info->invalidate();
    //FIXMEcache_block_info = NULL;
    if (cache_block_info)
    {
    ...
    hit_where = m_next_cache_cntlr->processInvalReqFromPrevCache(this, mem_op_type, ca_address, modeled, count, Prefetch::NONE, t_start, false);
...
HitWhere::where_t
CacheCntlr::processInvalReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
    bool cache_hit = operationPermissibleinCache(address, mem_op_type);
    HitWhere::where_t hit_where = HitWhere::MISS;
    SharedCacheBlockInfo* cache_block_info = getCacheBlockInfo(address);
    ...
    //FIXMELOG_ASSERT_ERROR(m_next_cache_cntlr != NULL, "Cannot do passthrough on an LLC");
    ...
    else
    {
        ...
        if (m_next_cache_cntlr)
        {
            // invalidate cache line before pass to next level cache
            if (getCacheState(address) != CacheState::INVALID)
            {
                invalidateCacheBlock(address);
            }
            // let the next cache level handle it.
            hit_where = m_next_cache_cntlr->processInvalReqFromPrevCache(this, mem_op_type, address, modeled, count, isPrefetch == Prefetch::NONE ? Prefetch::NONE : Prefetch::OTHER, t_issue, false);
        }
        else // last-level cache
        {
            // invalidate dst addr of CIM -> no miss
            hit_where = HitWhere::L3_OWN;
            LOG_PRINT("CLINVAL: done!\n");
            // do nothing
        }
    }
...
HitWhere::where_t
CacheCntlr::processCIMReqFromCore(
      Core::lock_signal_t lock_signal,
      Core::mem_op_t mem_op_type,
      IntPtr ca_address, UInt32 offset,
      Byte* data_buf, UInt32 data_length,
...
{
    HitWhere::where_t hit_where = HitWhere::MISS;
    Byte cim_buf[data_length];

    LOG_PRINT("processCIMReqFromCore(), lock_signal(%u), mem_op_type(%u), ca_address(0x%x)",
             lock_signal, mem_op_type, ca_address);
    ...
    if (m_master->m_dram_cntlr)
    {
        ...
        {
            case Core::NOT:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::NOT, ca_address, false, cim_buf);
                break;
            case Core::AND:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::AND, ca_address, false, cim_buf);
                break;
            case Core::OR:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::OR, ca_address, false, cim_buf);
                break;
            case Core::NAND:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::NAND, ca_address, false, cim_buf);
                break;
            case Core::NOR:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::NOR, ca_address, false, cim_buf);
                break;
            case Core::XOR:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::XOR, ca_address, false, cim_buf);
                break;
            case Core::CIMREAD:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::CIMREAD, ca_address, false, cim_buf);
                break;
            case Core::AND_S:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::AND_S, ca_address, false, cim_buf);
                break;
            case Core::OR_S:
                boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, dram_latency) = accessDRAM(Core::OR_S, ca_address, false, cim_buf);
                break;
            default:
                LOG_PRINT_ERROR("Unsupported CIM Op Type(%u)", mem_op_type);
        }
    }
    getMemoryManager()->incrElapsedTime(dram_latency, ShmemPerfModel::_USER_THREAD);
    // Insert the line. Be sure to use SHARED (READ operation) as appropriate (upgrades are free anyway), we don't want to have to write back clean lines
    //FIXMEinsertCacheBlock(ca_address, CacheState::SHARED, cim_buf, m_core_id, ShmemPerfModel::_USER_THREAD);
      ...
      bool count)
{
   ...
   // added by MSON
   else if (mem_op_type == Core::NOT || mem_op_type == Core::AND || mem_op_type == Core::OR 
           || mem_op_type == Core::NAND || mem_op_type == Core::NOR || mem_op_type == Core::XOR
           || mem_op_type == Core::CIMREAD || mem_op_type == Core::AND_S || mem_op_type == Core::OR_S)
   {
      cache_hit = false;
      //FIXMEcache_block_info->invalidate();
      //FIXMEcache_block_info = NULL;
      ...
      else // last-level cache
      {
         ...
         else if (m_master->m_dram_cntlr)
         {
            ...
            else
            {
               ...
               // added by MSON
               if (mem_op_type == Core::NOT || mem_op_type == Core::AND || mem_op_type == Core::OR || mem_op_type == Core::NAND || mem_op_type == Core::NOR || mem_op_type == Core::XOR
                       || mem_op_type == Core::CIMREAD || mem_op_type == Core::AND_S || mem_op_type == Core::OR_S)
               {
                   boost::tie<HitWhere::where_t, SubsecondTime>(hit_where, latency) = accessDRAM(mem_op_type, address, isPrefetch != Prefetch::NONE, data_buf);
               }
               else
               {
...
{
   LOG_PRINT("accessDRAM called!\n");
   // check CIM cmd here
   if (mem_op_type == Core::NOT)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received NOT cmd!\n");
   }
   if (mem_op_type == Core::AND)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received AND cmd!\n");
   }
   if (mem_op_type == Core::OR)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received OR cmd!\n");
   }
   if (mem_op_type == Core::NAND)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received NAND cmd!\n");
   }
   if (mem_op_type == Core::NOR)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received NOR cmd!\n");
   }
   if (mem_op_type == Core::XOR)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received XOR cmd!\n");
   }
   if (mem_op_type == Core::CIMREAD)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received CIMREAD cmd!\n");
   }
   if (mem_op_type == Core::AND_S)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received AND_S cmd!\n");
   }
   if (mem_op_type == Core::OR_S)
   {
       LOG_PRINT("CacheCntlr::accessDRAM received OR_S cmd!\n");
   }
   // check CIM cmd here
   ...
   switch (mem_op_type)
   {
      ...
      case Core::AND_S:
      case Core::OR_S:
         boost::tie(dram_latency, hit_where) = m_master->m_dram_cntlr->sendCimToMM(mem_op_type, address, m_core_id_master, data_buf, t_issue, m_shmem_perf);
         break;
      ...
      IntPtr address, Core::mem_op_t mem_op_type, CacheBlockInfo **cache_block_info)
{
   ...
   if (mem_op_type == Core::CLFLUSH)
       LOG_PRINT("CacheCntlr::operationPermissibleinCache received CLFLUSH cmd!\n");
   if (mem_op_type == Core::CLINVAL)
       LOG_PRINT("CacheCntlr::operationPermissibleinCache received CLINVAL cmd!\n");
   // end check CIM cmd here
   ...
   LOG_PRINT("address %lx state %c: permissible %d", address, CStateString(cstate), cache_hit);
