      ...
      Core::MemModeled modeled)
{
   ...
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received CLFLUSH cmd!\n");
       LOG_PRINT("Address of CLFLUSH op: %p\n", address);
       LOG_PRINT("Data length of CLFLUSH op: %d\n", data_length);
   }
   if (mem_op_type == Core::CLINVAL)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received CLINVAL cmd!\n");
       LOG_PRINT("Address of CLINVAL op: %p\n", address);
       LOG_PRINT("Data length of CLINVAL op: %d\n", data_length);
   }
   if (mem_op_type == Core::NOT)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received NOT cmd!\n");
       LOG_PRINT("Address of NOT op: %p\n", address);
       LOG_PRINT("Data length of NOT op: %d\n", data_length);
   }
   if (mem_op_type == Core::AND)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received AND cmd!\n");
       LOG_PRINT("Address of AND op: %p\n", address);
       LOG_PRINT("Data length of AND op: %d\n", data_length);
   }
   if (mem_op_type == Core::OR)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received OR cmd!\n");
       LOG_PRINT("Address of OR op: %p\n", address);
       LOG_PRINT("Data length of OR op: %d\n", data_length);
   }
   if (mem_op_type == Core::NAND)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received NAND cmd!\n");
       LOG_PRINT("Address of NAND op: %p\n", address);
       LOG_PRINT("Data length of NAND op: %d\n", data_length);
   }
   if (mem_op_type == Core::NOR)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received NOR cmd!\n");
       LOG_PRINT("Address of NOR op: %p\n", address);
       LOG_PRINT("Data length of NOR op: %d\n", data_length);
   }
   if (mem_op_type == Core::XOR)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received XOR cmd!\n");
       LOG_PRINT("Address of XOR op: %p\n", address);
       LOG_PRINT("Data length of XOR op: %d\n", data_length);
   }
   if (mem_op_type == Core::CIMREAD)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received CIMREAD cmd!\n");
       LOG_PRINT("Address of CIMREAD op: %p\n", address);
       LOG_PRINT("Data length of CIMREAD op: %d\n", data_length);
   }
   if (mem_op_type == Core::AND_S)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received AND_S cmd!\n");
       LOG_PRINT("Address of AND_S op: %p\n", address);
       LOG_PRINT("Data length of AND_S op: %d\n", data_length);
   }
   if (mem_op_type == Core::OR_S)
   {
       LOG_PRINT("MemoryManager::coreInitiateMemoryAccess received OR_S cmd!\n");
       LOG_PRINT("Address of OR_S op: %p\n", address);
       LOG_PRINT("Data length of OR_S op: %d\n", data_length);
   }
   ...
   // added by MSON
   if (mem_op_type == Core::CLFLUSH)
       return m_cache_cntlrs[mem_component]->processFlushReqFromCore(
               lock_signal,
               mem_op_type,
               address, offset,
   ...
   else if (mem_op_type == Core::CLINVAL)
       return m_cache_cntlrs[mem_component]->processInvalReqFromCore(
               lock_signal,
               mem_op_type,
               address, offset,
               data_buf, data_length,
   ...
   {
       //FIXMESubsecondTime t_start = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
       
       HitWhere::where_t cache = m_cache_cntlrs[MemComponent::L3_CACHE]->processCIMReqFromCore(
               lock_signal,
               mem_op_type,
               address, offset,
               data_buf, data_length,
       ...
       //FIXMEHitWhere::where_t cache = m_cache_cntlrs[mem_component]->processMemOpFromCore(
       //FIXME        lock_signal,
       //FIXME        mem_op_type,
       //FIXME        address, offset,
       //FIXME        data_buf, data_length,
       //FIXME        modeled == Core::MEM_MODELED_NONE || modeled == Core::MEM_MODELED_COUNT ? false : true,
       //FIXME        modeled == Core::MEM_MODELED_NONE ? false : true);
       
       //FIXMESubsecondTime t_end = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
       //FIXMEstd::cout << t_start - t_end << std::endl;
       //FIXMESubsecondTime m_and_cost = SubsecondTime::FS() * static_cast<uint64_t>(TimeConverter<float>::NStoFS(Sim()->getCfg()->getFloat("perf_model/dram/and")));
       //FIXMEstd::cout << m_nvm_and_in_latch_cost << std::endl;
       //FIXMEgetShmemPerfModel()->setElapsedTime(ShmemPerfModel::_USER_THREAD, t_end);
       return cache; 
   }
   else
   ...
   //FIXMEreturn m_cache_cntlrs[mem_component]->processMemOpFromCore(
   //FIXME      lock_signal,
   //FIXME      mem_op_type,
   //FIXME      address, offset,
   //FIXME      data_buf, data_length,
