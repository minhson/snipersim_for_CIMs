      ...
      SubsecondTime now)
{
   ...
   {
       LOG_PRINT("Core::initiateMemoryAccess received CLFLUSH cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "CLFLUSH", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::CLINVAL)
   {
       LOG_PRINT("Core::initiateMemoryAccess received CLINVAL cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "CLINVAL", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::NOT)
   {
       LOG_PRINT("Core::initiateMemoryAccess received NOT cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "NOT", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::AND)
   {
       LOG_PRINT("Core::initiateMemoryAccess received AND cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "AND", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::OR)
   {
       LOG_PRINT("Core::initiateMemoryAccess received OR cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "OR", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::NAND)
   {
       LOG_PRINT("Core::initiateMemoryAccess received NAND cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "NAND", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::NOR)
   {
       LOG_PRINT("Core::initiateMemoryAccess received NOR cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "NOR", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::XOR)
   {
       LOG_PRINT("Core::initiateMemoryAccess received XOR cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "XOR", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::CIMREAD)
   {
       LOG_PRINT("Core::initiateMemoryAccess received CIMREAD cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "CIMREAD", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::AND_S)
   {
       LOG_PRINT("Core::initiateMemoryAccess received AND_S cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "AND_S", ' ', ModeledString(modeled));
   }
   if (mem_op_type == Core::OR_S)
   {
       LOG_PRINT("Core::initiateMemoryAccess received OR_S cmd!\n");
       LOG_PRINT("access %lx+%u %s%c modeled(%s)\n", address, data_size, "OR_S", ' ', ModeledString(modeled));
   }
   ...
   // added by MSON
   if (mem_op_type == NOT || mem_op_type == AND || mem_op_type == OR 
           || mem_op_type == NAND || mem_op_type == NOR || mem_op_type == XOR || mem_op_type == CIMREAD
           || mem_op_type == AND_S || mem_op_type == OR_S)
   {
       //FIXMEprintf("begin_addr: %x\n", begin_addr);
       //FIXMEprintf("end_addr: %x\n", end_addr);
       //FIXMEprintf("begin_addr_aligned: %x\n", begin_addr_aligned);
       //FIXMEprintf("end_addr_aligned: %x\n", end_addr_aligned);
       for (IntPtr curr_addr_aligned = begin_addr_aligned; curr_addr_aligned < end_addr_aligned; curr_addr_aligned += cache_block_size)
       {
           ...
           // set offset for CIM is 0???
           curr_offset = 0;

           // Determine the size
           // set curr_size = cache_block_size for CIM???
           curr_size = cache_block_size - (curr_offset);
        
           //FIXMEprintf("curr_offset: %u\n", curr_offset);
           //FIXMEprintf("curr_size: %u\n", curr_size);
           //FIXMEprintf("curr_addr_aligned: %x\n", curr_addr_aligned);  
           LOG_PRINT("Sending CIM cmd to MM (ADDR(0x%x), size(%u))!\n", address, data_size);
           ...
           LOG_PRINT("End sending CIM cmd to MM (ADDR(0x%x), size(%u))!\n", address, data_size);
...
Core::accessMemory(lock_signal_t lock_signal, mem_op_t mem_op_type, IntPtr d_addr, char* data_buffer, UInt32 data_size, MemModeled modeled, IntPtr eip, SubsecondTime now, bool is_fault_mask)
{
   ...
   if (mem_op_type == Core::CLFLUSH)
   {
       LOG_PRINT("Core::accessMemory received CLFLUSH cmd!\n");
   }
   if (mem_op_type == Core::CLINVAL)
   {
       LOG_PRINT("Core::accessMemory received CLINVAL cmd!\n");
   }
   if (mem_op_type == Core::NOT)
   {
       LOG_PRINT("Core::accessMemory received NOT cmd!\n");
   }
   if (mem_op_type == Core::AND)
   {
       LOG_PRINT("Core::accessMemory received AND cmd!\n");
   }
   if (mem_op_type == Core::OR)
   {
       LOG_PRINT("Core::accessMemory received OR cmd!\n");
   }
   if (mem_op_type == Core::NAND)
   {
       LOG_PRINT("Core::accessMemory received NAND cmd!\n");
   }
   if (mem_op_type == Core::NOR)
   {
       LOG_PRINT("Core::accessMemory received NOR cmd!\n");
   }
   if (mem_op_type == Core::XOR)
   {
       LOG_PRINT("Core::accessMemory received XOR cmd!\n");
   }
   if (mem_op_type == Core::AND_S)
   {
       LOG_PRINT("Core::accessMemory received AND_S cmd!\n");
   }
   if (mem_op_type == Core::OR_S)
   {
       LOG_PRINT("Core::accessMemory received OR_S cmd!\n");
   }
   // end check CIM cmd here
