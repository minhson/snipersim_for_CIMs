...
bool handleAccessMemory(void *arg, Sift::MemoryLockType lock_signal, Sift::MemoryOpType mem_op, uint64_t d_addr, uint8_t* data_buffer, uint32_t data_size)
{
   ...
   else if (mem_op == Sift::MemWrite || mem_op == Sift::MemCLFLUSH || mem_op == Sift::MemCLINVAL || mem_op == Sift::MemNOT || mem_op == Sift::MemAND || mem_op == Sift::MemOR || mem_op == Sift::MemNAND || mem_op == Sift::MemNOR || mem_op == Sift::MemXOR || mem_op == Sift::MemCIMREAD || mem_op == Sift::MemAND_S || mem_op == Sift::MemOR_S)
   {
      ...
      {
          printf("syscall_modeling.cc handleAccessMemory received CLFLUSH cmd!\n");
      }
      if (mem_op == Sift::MemCLINVAL)
      {
          printf("syscall_modeling.cc handleAccessMemory received CLINVAL cmd!\n");
      }
      if (mem_op == Sift::MemNOT)
      {
          printf("syscall_modeling.cc handleAccessMemory received NOT cmd!\n");
      }
      if (mem_op == Sift::MemAND)
      {
          printf("syscall_modeling.cc handleAccessMemory received AND cmd!\n");
      }
      if (mem_op == Sift::MemOR)
      {
          printf("syscall_modeling.cc handleAccessMemory received OR cmd!\n");
      }
      if (mem_op == Sift::MemNAND)
      {
          printf("syscall_modeling.cc handleAccessMemory received NAND cmd!\n");
      }
      if (mem_op == Sift::MemNOR)
      {
          printf("syscall_modeling.cc handleAccessMemory received NOR cmd!\n");
      }
      if (mem_op == Sift::MemXOR)
      {
          printf("syscall_modeling.cc handleAccessMemory received XOR cmd!\n");
      }
      if (mem_op == Sift::MemCIMREAD)
      {
          printf("syscall_modeling.cc handleAccessMemory received CIMREAD cmd!\n");
      }
      if (mem_op == Sift::MemAND_S)
      {
          printf("syscall_modeling.cc handleAccessMemory received AND_S cmd!\n");
      }
      if (mem_op == Sift::MemOR_S)
      {
          printf("syscall_modeling.cc handleAccessMemory received OR_S cmd!\n");
      }
