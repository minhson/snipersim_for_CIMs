...
bool Sift::Reader::AccessMemory(MemoryLockType lock_signal, MemoryOpType mem_op, uint64_t d_addr, uint8_t *data_buffer, uint32_t data_size)
{
   ...
   else if (mem_op == MemWrite || mem_op == MemCLFLUSH || mem_op == MemCLINVAL || mem_op == MemNOT || mem_op == MemAND || mem_op == MemOR || mem_op == MemNAND || mem_op == MemNOR || mem_op == MemXOR)
   {
      if (mem_op == MemCLFLUSH)
      {
          printf("Sift::Reader::AccessMemory received CLFLUSH cmd!\n");
      }
      if (mem_op == MemCLINVAL)
      {
          printf("Sift::Reader::AccessMemory received CLINVAL cmd!\n");
      }
      if (mem_op == MemNOT)
      {
          printf("Sift::Reader::AccessMemory received NOT cmd!\n");
      }
      if (mem_op == MemAND)
      {
          printf("Sift::Reader::AccessMemory received AND cmd!\n");
      }
      if (mem_op == MemOR)
      {
          printf("Sift::Reader::AccessMemory received OR cmd!\n");
      }
      if (mem_op == MemNAND)
      {
          printf("Sift::Reader::AccessMemory received NAND cmd!\n");
      }
      if (mem_op == MemNOR)
      {
          printf("Sift::Reader::AccessMemory received NOR cmd!\n");
      }
      if (mem_op == MemXOR)
      {
          printf("Sift::Reader::AccessMemory received XOR cmd!\n");
      }
