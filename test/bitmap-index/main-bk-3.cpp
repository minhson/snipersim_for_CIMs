...
{
    int weeks = 4;
    int users = 16000000; // 8 million or 16 million users
    ...
    if (genderCol != 0) {
        ibis::index *genderIdx = ibis::index::create(genderCol, 0, 0, 1);
        if (genderIdx != 0){
            // gender is a pointer to the newly created gender index 
            // Returns the number of bit vectors used by the index 
            //FIXMEstd::cout << "number of bit vectors used by the gender: " << genderIdx->numBitvectors() << std::endl;
            //FIXMEstd::cout << "size in bytes: " << genderIdx->sizeInBytes() << std::endl;
            ...
            if (numMales >= 0) {
                ...
                //FIXMEstd::cout << "the query was successful" << std::endl;
                //FIXMEstd::cout << "numMales: " << numMales << std::endl;
            } else {
                // an error occurred while evaluating the query
                std::cout << "an error occurred while evaluating the query" << std::endl;
            }
            
            //FIXMEif (males.isCompressed())
            //FIXME    std::cout << "males is compressed" << std::endl;
            //FIXMEelse
            //FIXME    std::cout << "males is decompressed" << std::endl;
            //FIXMEstd::cout << "number of bits in males: " << males.size() << std::endl;
            //FIXMEstd::cout << "size in bytes of males: " << males.bytes() << std::endl;
        }
        delete genderIdx;
    }
    ...
    for (int i = 0; i < 7*weeks; i++) {
        ...
        if (dateCol[i] != 0) {
            dateIdx[i] = ibis::index::create(dateCol[i], 0, 0, 1);
            if (dateIdx[i] != 0) {
                // dateIdx[i] is a pointer to the newly created dateIdx[i] index 
                // Returns the number of bit vectors used by the index 
                //FIXMEstd::cout << "number of bit vectors used by the dateIdx" + std::to_string(i + 1) << ": " << dateIdx[i]->numBitvectors() << std::endl;
                //FIXMEstd::cout << "size in bytes: " << dateIdx[i]->sizeInBytes() << std::endl;
                // Prints human readable information.  Outputs information about the
                // index as text to the specified output stream.
                ...
                if (numYeses >= 0) {
                    ...
                    // the hits are stored in the 'males' bitvector
                    //FIXMEstd::cout << "the query was successful" << std::endl;
                    //FIXMEstd::cout << "numYeses: " << numYeses << std::endl;
                } else {
                    // an error occurred while evaluating the query
                    std::cout << "an error occurred while evaluating the query" << std::endl;
                }
                
                //FIXMEif (dateBv[i].isCompressed())
                //FIXME    std::cout << "dateBv" + std::to_string(i + 1) << " is compressed" << std::endl;
                //FIXMEelse
                //FIXME    std::cout << "dateBv" + std::to_string(i + 1) << " is decompressed" << std::endl;
                //FIXMEstd::cout << "number of bits in dateBv" + std::to_string(i + 1) << ": " << dateBv[i].size() << std::endl;
                //FIXMEstd::cout << "size in bytes of dateBv" + std::to_string(i + 1) << ": " << dateBv[i].bytes() << std::endl;
            }
            delete dateIdx[i];
        std::cout << "////////////////////////////////////////////////////////////////////////////////////////////////////" << std::endl;
        }  
