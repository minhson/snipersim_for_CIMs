...
int main()
{
    ...
    // add params for CIM
    int size_in_byte = users / 8; // size of bitvector in byte
    int row_buffer_size = std::pow(2, 16) / 8; // row buffer size in byte (8KB)
    ...
    for (int i = 0; i < weeks; i++) {
        CLINVAL((uint64_t) &w[i], (uint64_t) (row_buffer_size*split));
        // w[i] = dateBv[0] | dateBv[1]
        OR((uint64_t) &w[i], (uint64_t) (64*split)); // C=A|B 
        for (int j = 2; j < 7; j++) {
            // w[i] |= dateBv[i]
            OR_S((uint64_t) &w[i], (uint64_t) (64*split)); // A=A|B (S means shared address)
        }
    ...
    CLINVAL((uint64_t) &result1, (uint64_t) (row_buffer_size*split));
    // result = w[0] & w[1]
    AND((uint64_t) &result1, (uint64_t) (64*split)); // C=A&B
    // for loop in case weeks > 2
    for (int i = 2; i < weeks; i++) {
        AND_S((uint64_t) &result1, (uint64_t) (64*split)); // A=A&B (S means shared address)
    } 
    
    // read out the final result bitmap for counting bit 1
    READ((uint64_t) &result1, (uint64_t) (row_buffer_size*split));
    ...
    for (int i = 0; i < weeks; i++) {
        CLINVAL((uint64_t) &result2[i], (uint64_t) (row_buffer_size*split));
        // result2 = males & w[i]
        AND((uint64_t) &result2[i], (uint64_t) (64*split)); // C=A&B   
        // read out the final result bitmap for counting bit 1
        READ((uint64_t) &result2[i], (uint64_t) (row_buffer_size*split));
        // mimic bitcount
        count2[i] = males.cnt();
    }

    //FIXMEfor (int i = 0; i < weeks; i++) {
    //FIXME    result2[i] = males & w[i];
    //FIXME    count2[i] = result2[i]->cnt();
    //FIXME}
