...
//FIXMEint main(int argc, char** argv) {
//FIXME    if (argc <= 1) {
//FIXME        usage(*argv);
//FIXME        return 0;
//FIXME    }
//FIXME#if defined(_WIN32) && defined(_MSC_VER) && defined(_DEBUG)
//FIXME    // macro to turn on reporting of memory leaks
//FIXME    _CrtSetDbgFlag ( _CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF );
//FIXME#endif
//FIXME    try {
//FIXME        int interactive;
//FIXME        std::vector<const char*> alist, qlist, slist;
//FIXME        ibis::joinlist joins;
//FIXME        std::vector<std::string> queff; // queries read from files (-f)
//FIXME        const char* uid = ibis::util::userName();
//FIXME        ibis::horometer timer; // total elapsed time
//FIXME        timer.start();
//FIXME
//FIXME        // parse the command line arguments
//FIXME        parse_args(argc, argv, interactive, alist, slist, qlist,
//FIXME                   queff, joins);
//FIXME
//FIXME        // append new data one directory at a time, the same directory may
//FIXME        // be used many times, all incoming directories appended to the
//FIXME        // same output directory specified by appendTarget
//FIXME        for (std::vector<const char*>::const_iterator it = alist.begin();
//FIXME             it != alist.end();
//FIXME             ++ it) { // add new data before doing anything else
//FIXME            doAppend(*it);
//FIXME        }
//FIXME        alist.clear(); // no more use for it
//FIXME
//FIXME        if (yankstring != 0 && *yankstring != 0)
//FIXME            doDeletion();
//FIXME        if (keepstring != 0 && *keepstring != 0)
//FIXME            reverseDeletion();
//FIXME
//FIXME        // build new indexes
//FIXME        if (build_index > 0 && ! ibis::datasets.empty()) {
//FIXME            if (ibis::gVerbose > 0) {
//FIXME                ibis::util::logger lg;
//FIXME                lg() << *argv << ": start building indexes (nthreads="
//FIXME                     << build_index << ", indexingOption";
//FIXME                if (indexingOptions.empty()) {
//FIXME                    lg() << "= -";
//FIXME                }
//FIXME                else if (indexingOptions.size() == 1) {
//FIXME                    lg() << "= " << indexingOptions.back();
//FIXME                }
//FIXME                else {
//FIXME                    lg() << "s= {";
//FIXME                    for (unsigned j = 0; j+1 < indexingOptions.size(); j += 2)
//FIXME                        lg() << (j>0?", ":"") << indexingOptions[j] << ':'
//FIXME                             << indexingOptions[j+1];
//FIXME                    if (indexingOptions.size() % 2 > 0)
//FIXME                        lg() << ", *:" << indexingOptions.back();
//FIXME                    lg() << "}";
//FIXME                }
//FIXME                lg() << ") ...";
//FIXME            }
//FIXME
//FIXME            ibis::horometer timer1;
//FIXME            timer1.start();
//FIXME            for (ibis::partList::const_iterator it = ibis::datasets.begin();
//FIXME                 it != ibis::datasets.end(); ++ it) {
//FIXME                if (indexingOptions.size() == 1 &&
//FIXME                    ((*it)->indexSpec() == 0 ||
//FIXME                     stricmp(indexingOptions.back(), (*it)->indexSpec())
//FIXME                     != 0)) {
//FIXME                    (*it)->indexSpec(indexingOptions.back());
//FIXME                    (*it)->purgeIndexFiles();
//FIXME                }
//FIXME                else if (zapping) {
//FIXME                    (*it)->purgeIndexFiles();
//FIXME                }
//FIXME                (*it)->buildIndexes(indexingOptions, build_index);
//FIXME            }
//FIXME            timer1.stop();
//FIXME            if (ibis::gVerbose >= 0) {
//FIXME                ibis::util::logger lg;
//FIXME                lg() << *argv << ": building indexes for "
//FIXME                     << ibis::datasets.size() << " data partition"
//FIXME                     << (ibis::datasets.size()>1 ? "s" : "");
//FIXME                if (ibis::gVerbose > 0)
//FIXME                    lg() << " took " << timer1.CPUTime() << " CPU seconds, "
//FIXME                         << timer1.realTime() << " elapsed seconds\n";
//FIXME            }
//FIXME            zapping = false;
//FIXME        }
//FIXME        // sort the specified columns
//FIXME        if (slist.size() > 0) {
//FIXME            ibis::horometer timer2;
//FIXME            timer2.start();
//FIXME            for (ibis::partList::const_iterator it = ibis::datasets.begin();
//FIXME                 it != ibis::datasets.end(); ++ it) {
//FIXME                for (size_t j = 0; j < slist.size(); ++ j)
//FIXME                    (*it)->buildSorted(slist[j]);
//FIXME            }
//FIXME            timer2.stop();
//FIXME            if (ibis::gVerbose >= 0) {
//FIXME                ibis::util::logger lg;
//FIXME                lg() << *argv << ": building sorted version of "
//FIXME                     << slist.size() << " column" << (slist.size()>1 ? "s" : "")
//FIXME                     << " for " << ibis::datasets.size() << " data partition"
//FIXME                     << (ibis::datasets.size()>1 ? "s" : "");
//FIXME                if (ibis::gVerbose > 0)
//FIXME                    lg() << " took " << timer2.CPUTime() << " CPU seconds, "
//FIXME                         << timer2.realTime() << " elapsed seconds\n";
//FIXME            }
//FIXME            slist.clear(); // no longer needed
//FIXME        }
//FIXME
//FIXME        if (testing > 0 && ! ibis::datasets.empty() && threading > 0 &&
//FIXME            qlist.empty()) { // generate random queries
//FIXME            const unsigned mq = 
//FIXME                (testing > (threading+threading) ? testing : (threading+threading));
//FIXME            randomQueries(*ibis::datasets[0], mq, qlist, queff);
//FIXME        }
//FIXME        else if (testing > 0 && ! ibis::datasets.empty()) {
//FIXME            // performing self test
//FIXME            LOGGER(ibis::gVerbose > 0) << *argv << ": start testing ...";
//FIXME            ibis::horometer timer3;
//FIXME            timer3.start();
//FIXME            for (ibis::partList::const_iterator it = ibis::datasets.begin();
//FIXME                 it != ibis::datasets.end(); ++ it) {
//FIXME                // tell the partition to perform self tests
//FIXME                long nerr = (*it)->selfTest(testing);
//FIXME                (*it)->unloadIndexes();
//FIXME
//FIXME                if (ibis::gVerbose >= 0) {
//FIXME                    ibis::util::logger lg;
//FIXME                    lg() << "self tests on " << (*it)->name();
//FIXME                    if (nerr == 0)
//FIXME                        lg() << " found no error";
//FIXME                    else if (nerr == 1)
//FIXME                        lg() << " found 1 error";
//FIXME                    else if (nerr > 1)
//FIXME                        lg() << " found " << nerr << " errors";
//FIXME                    else
//FIXME                        lg() << " returned unexpected value " << nerr;
//FIXME                }
//FIXME            }
//FIXME            timer3.stop();
//FIXME            LOGGER(ibis::gVerbose > 0)
//FIXME                << *argv << ": testing " << ibis::datasets.size()
//FIXME                << " data partition" << (ibis::datasets.size()>1 ? "s" : "")
//FIXME                << " took " << timer3.CPUTime() << " CPU seconds, "
//FIXME                << timer3.realTime() << " elapsed seconds\n";
//FIXME        }
//FIXME
//FIXME        if (ibis::datasets.empty() && !qlist.empty()) {
//FIXME            LOGGER(ibis::gVerbose >= 0)
//FIXME                << *argv << " must have at least one data partition "
//FIXME                "to process any query.";
//FIXME        }
//FIXME        else if (qlist.size() > 1 && threading > 1) {
//FIXME#if defined(_DEBUG) || defined(DEBUG)
//FIXME            for (std::vector<const char*>::const_iterator it = qlist.begin();
//FIXME                 it != qlist.end(); ++it) {
//FIXME                parseString(uid, *it, ibis::datasets);
//FIXME            }
//FIXME#else
//FIXME            // process queries in a thread pool
//FIXME            const int nth =
//FIXME                (threading <= qlist.size() ? threading : qlist.size()) - 1;
//FIXME            ibis::util::counter taskpool;
//FIXME            thArg args(uid, qlist, taskpool);
//FIXME            std::vector<pthread_t> tid(nth);
//FIXME            LOGGER(ibis::gVerbose > 1)
//FIXME                << *argv << " will process " << qlist.size() << " quer"
//FIXME                << (qlist.size()>1?"ies":"y") << " using " << nth+1 << " thread"
//FIXME                << (nth>0?"s":"");
//FIXME            for (int i = 0; i < nth; ++ i) { //
//FIXME                int ierr = pthread_create(&(tid[i]), 0, thFun, (void*)&args);
//FIXME                if (ierr != 0) {
//FIXME                    LOGGER(ibis::gVerbose >= 0)
//FIXME                        << "Warning -- pthread_create failed to create thread "
//FIXME                        << i << " -- " << strerror(ierr);
//FIXME                    return(-5);
//FIXME                }
//FIXME            }
//FIXME            thFun((void*)&args); // this thread does something too
//FIXME            for (int i = 0; i < nth; ++ i) {
//FIXME                void *status;
//FIXME                int ierr = pthread_join(tid[i], (void**)&status);
//FIXME                LOGGER(ibis::gVerbose >= 0 && ierr != 0)
//FIXME                    << "pthread_join failed on thread " << i
//FIXME                    << " -- " << strerror(ierr);
//FIXME            }
//FIXME#endif
//FIXME            queff.clear();
//FIXME            qlist.clear();
//FIXME        }
//FIXME        else if (qlist.size() > 0) { // no new threads
//FIXME            for (std::vector<const char*>::const_iterator it = qlist.begin();
//FIXME                 it != qlist.end(); ++it) {
//FIXME                parseString(uid, *it, ibis::datasets);
//FIXME            }
...
//FIXME            qlist.clear();
//FIXME        }
//FIXME        else if (ridfile != 0) {
//FIXME            for (ibis::partList::iterator itt = ibis::datasets.begin();
//FIXME                 itt != ibis::datasets.end();
//FIXME                 ++ itt)
//FIXME                doQuery((*itt), uid, 0, 0, 0, 0, 0);
//FIXME        }
//FIXME        ridfile = 0;
//FIXME
//FIXME        // process the joins one at a time
//FIXME        for (size_t j = 0; j < joins.size(); ++j) {
//FIXME            doJoin(uid, *joins[j], ibis::datasets);
//FIXME        }
//FIXME
//FIXME        if (interactive) {      // interactive operations
//FIXME            std::string str;
//FIXME            if (ibis::gVerbose >= 0) {
//FIXME                // entering the interactive mode, print the help message
...
//FIXME                    clean_up(true);
//FIXME                    return(0);
//FIXME                case 'p': // print command
//FIXME                case 'P':
//FIXME                    print(str.c_str()); break;
//FIXME                case 's': // a select statement
//FIXME                case 'f':
//FIXME                case 'w':
//FIXME                case 'S':
//FIXME                case 'F':
//FIXME                case 'W': {
//FIXME                    parseString(uid, str.c_str(), ibis::datasets);
//FIXME                    break;}
//FIXME                case 'a':
//FIXME                case 'A': {
//FIXME                    const char* dir = str.c_str();
//FIXME                    while(std::isalpha(*dir)) ++dir; // skip key word append
//FIXME                    while(std::isspace(*dir)) ++dir; // skip space
//FIXME                    doAppend(dir);
//FIXME                    break;}
//FIXME                }
//FIXME            }
...
//FIXME
//FIXME        timer.stop();
//FIXME        LOGGER(timer.realTime() > 0.001 && ibis::gVerbose > 0)
//FIXME            << *argv << " -- total CPU time " << timer.CPUTime()
//FIXME            << " s, total elapsed time " << timer.realTime() << " s";
//FIXME
//FIXME        clean_up(true);
//FIXME        // last thing -- close the file logging the messages
//FIXME        //ibis::util::closeLogFile();
//FIXME        return 0;
//FIXME    }
//FIXME    catch (const std::exception& e) {
//FIXME        LOGGER(ibis::gVerbose >= 0)
//FIXME            << "Warning ** " << *argv
//FIXME            << " received a standard exception\n" << e.what();
//FIXME        return -10;
//FIXME    }
//FIXME    catch (const char* s) {
//FIXME        LOGGER(ibis::gVerbose >= 0)
//FIXME            << "Warning ** " << *argv
//FIXME            << " received a string exception\n" << s;
//FIXME        return -11;
//FIXME    }
//FIXME    catch (...) {
//FIXME        LOGGER(ibis::gVerbose >= 0)
//FIXME            << "Warning ** " << *argv
//FIXME            << " received an unexpected exception";
//FIXME        return -12;
//FIXME    }
//FIXME} // main
